...
object LocalLdaExample {
  ...
  private def run(params: Params) {
    ...
    if(ldaModel.isInstanceOf[LocalLDAModel]) {
      ...
      localLDAModel.save(sc, C.ldaModelSavePath)
    ...
    // save topic to hdfs
    val result = topicIndices.map {
      case (terms, termWeights) =>
        terms.zip(termWeights).map {
          case (term, weight) => s"$term:$weight"
        }.mkString("\t")
    }
    sc.parallelize(result, 1).saveAsTextFile(C.trainTopicResultPath)
  ...
  private def preprocess(
    ...
    sc.parallelize(vocab.toArray.toList).saveAsTextFile(C.trainVocabPath)
